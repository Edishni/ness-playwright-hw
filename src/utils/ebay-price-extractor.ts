...
/**
 * Get price from eBay locator element
 * 
 * Safely extracts and parses price from a Playwright locator
 * 
 * @param locator - Playwright locator for price element
 * @returns Extracted price information or null if extraction fails
 */
export async function getPriceFromLocator(locator: Locator): Promise<ExtractedPrice | null> {
  try {
    const priceText = await locator.textContent({ timeout: 5000 });
    return extractPrice(priceText || '');
  } catch (error) {
    console.warn(`Failed to extract price from locator: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return null;
  }
}
 ...
 * 
 * @param price - Extracted price object
 * @returns Formatted string like "ILS 281.32" or "ILS 65.26 - ILS 98.05"
 */
export function formatPrice(price: ExtractedPrice): string {
  if (price.isRange) {
    return `${price.currency} ${price.minPrice} - ${price.currency} ${price.maxPrice}`;
  }
  return `${price.currency} ${price.minPrice}`;
}
 ...
 */
export function logPrice(price: ExtractedPrice, itemName?: string): void {
  const context = itemName ? ` for "${itemName}"` : '';
  if (price.isRange) {
    console.log(`Price range${context}: ${formatPrice(price)}`);
  } else {
    console.log(`Price${context}: ${formatPrice(price)}`);
  }
}
